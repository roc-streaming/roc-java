 ...
 * <p>
 * Each slot of a peer (see {@link Slot} ) has multiple interfaces, one for each of the interface
 * types. The user interconnects peers by binding an interface of one peer to an URI, and then
 * connecting the corresponding interface of another peer to that URI.
 * <p>
 * A URI is represented by {@link Endpoint} object.
 * <p>
 * The interface defines the type of the communication with the remote peer and the set of
 ...
 * negotiation, etc. When a consolidated connection is established, peers may automatically setup
 ...
 * <p>
 * {@link Interface#AUDIO_CONTROL} is a lower-level bidirectional interface for control streams.
 * If you use {@link Interface#AUDIO_SOURCE} and {@link Interface#AUDIO_REPAIR}, you usually also
 * need to use {@link Interface#AUDIO_CONTROL} to enable carrying additional non-transport
 * information.
 */
